...
public:
	...
	void clients0(FifoCfg name, Config config, size_t time, int nBates, int nWrite, int nBatesServer, std::mutex& mtx)
	{
		Client client(name, config, time, time);
		std::string dataClient = "";
		...
		std::cerr << "111" << std::endl;
		// ASSERT_THROW(WriteClient(client, nBates, nWrite), std::runtime_error);

		sleep(3);
		//client.stop();
		ASSERT_THROW(client.stop(), std::runtime_error);
		std::cerr << "11100" << std::endl;
		std::cerr << "11100" << std::endl;
	...
	void servers0(std::list<FifoCfg> fifoCfg, Config config, size_t time, int nBates, int nWrite, size_t nClient,
	              int nBatesClient, std::mutex& mtx0)
	{
		std::string dataServer = "";
		auto getterServer      = [&dataServer, &mtx0](size_t id, FifoRead::Data&& dataq) {
...
// TEST_F(ServerClientTest, server_сrashed_NQW)
//{
//	FifoCfg k1{FIFO1, FIFO1 + "_reverse"};
//	FifoCfg k2{FIFO2, FIFO2 + "_reverse"};
//	FifoCfg k3{FIFO3, FIFO3 + "_reverse"};
...
//	std::mutex mtx0;
//	std::thread tServer([data, sizeNServer, nWriteServer, dataClient, this, &mtx0]() {
//		servers0(data, Ipc::Config::NQW, 2000, sizeNServer, nWriteServer, data.size(), dataClient, mtx0);
//	});
//	std::mutex mtx;
//	std::thread tClient1([&k1, &sizeNClient, nWriteClient, dataServer, this, &mtx]() {
//		clients0(k1, Ipc::Config::NQW, 2000, sizeNClient, nWriteClient, dataServer, mtx);
//	});
//
//	std::thread tClient2([&k2, &sizeNClient, nWriteClient, dataServer, this, &mtx]() {
//		clients0(k2, Ipc::Config::NQW, 2000, sizeNClient, nWriteClient, dataServer, mtx);
//	});
//
//	std::thread tClient3([&k3, &sizeNClient, nWriteClient, dataServer, this, &mtx]() {
//		clients0(k3, Ipc::Config::NQW, 2000, sizeNClient, nWriteClient, dataServer, mtx);
//	});
//
//	tClient1.join();
//	tClient2.join();
...
//	tServer.join();
// }
TEST_F(ServerClientTest, server_сrashed_QW)
{
	FifoCfg k1{FIFO1, FIFO1 + "_reverse"};
	FifoCfg k2{FIFO2, FIFO2 + "_reverse"};
	FifoCfg k3{FIFO3, FIFO3 + "_reverse"};
	...
	std::mutex mtx0;
	std::thread tServer([data, sizeNServer, nWriteServer, dataClient, this, &mtx0]() {
		servers0(data, Ipc::Config::QW, 0, sizeNServer, nWriteServer, data.size(), dataClient, mtx0);
	});
	std::mutex mtx;
	std::thread tClient1([&k1, &sizeNClient, nWriteClient, dataServer, this, &mtx]() {
		clients0(k1, Ipc::Config::QW, 0, sizeNClient, nWriteClient, dataServer, mtx);
	});

	std::thread tClient2([&k2, &sizeNClient, nWriteClient, dataServer, this, &mtx]() {
		clients0(k2, Ipc::Config::QW, 0, sizeNClient, nWriteClient, dataServer, mtx);
	});

	std::thread tClient3([&k3, &sizeNClient, nWriteClient, dataServer, this, &mtx]() {
		clients0(k3, Ipc::Config::QW, 0, sizeNClient, nWriteClient, dataServer, mtx);
	});
