...
TEST(Server_Client, 1)
{
	...
	auto getterServer = [&dataServer](FifoCfg name, FifoRead::Data&& dataq) {
...
std::string data1(n, 'v');
for(int i = 0; i != 5; i++) {
	client1.write((void*)data1.data(),n);
	sleep(1);
}
sleep(9);
client1.stop();
...
std::thread tClient2([&client2, &n]() {
	...
	std::string data1(n, 'v');
	for(int i = 0; i != 5; i++) {
		client2.write((void*)data1.data(),n);
		sleep(1);
	}
	sleep(9);
	client2.stop();
...
ASSERT_TRUE(dataServer.size() == n*10);
